...
export async function POST(request:Request){
  try{if(!rateLimit(request,"public-appointment",6,60*60*1000))return NextResponse.json({error:"Trop de demandes. Réessayez plus tard."},{status:429});const body=await request.json() as Record<string,unknown>;if(requiredText(body.website))return NextResponse.json({ok:true},{status:201});
    const input={name:requiredText(body.name,120),email:requiredText(body.email,180),phone:requiredText(body.phone,50),address:requiredText(body.address,300),requestType:requiredText(body.requestType,40),reason:requiredText(body.reason,2000),preferredDate:requiredText(body.preferredDate,10),preferredTimeStart:requiredText(body.preferredTimeStart,5),preferredTimeEnd:"",alternateDate:"",alternateTimeStart:"",alternateTimeEnd:"",notes:requiredText(body.notes,2000)};
    if(!input.name||!isValidEmail(input.email)||!input.phone||!input.address||!allowedTypes.has(input.requestType)||!input.reason||!isIsoDate(input.preferredDate)||!/^\d{2}:\d{2}$/.test(input.preferredTimeStart)||body.consent!==true)return NextResponse.json({error:"Veuillez vérifier les champs obligatoires."},{status:400});
