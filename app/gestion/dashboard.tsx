...
export function Dashboard({initialAppointments, initialSelectedId}: {initialAppointments: Appointment[]; initialSelectedId?: string}) {
  ...
  async function move(item: Appointment, day: string) {
    ...
    if (item.endsAt) {
      ...
      end = localInput(new Date(new Date(`${start}:00`).getTime() + duration).toISOString());
    }
    const response = await fetch(`/api/gestion/rendez-vous/${item.id}`, {method: "PATCH", headers: {"content-type": "application/json"}, body: JSON.stringify({action: "propose", startsAt: start, endsAt: end, adminNotes: item.adminNotes || ""})});
    if (response.ok) {
...
function AppointmentPanel({appointment, onClose, onSaved, onDeleted, notice, setNotice}: {appointment: Appointment; onClose: () => void; onSaved: (item: Appointment) => void; onDeleted: () => void; notice: string; setNotice: (value: string) => void}) {
  ...
  const [start, setStart] = useState(localInput(appointment.startsAt) || availabilityStart(appointment.preferredDate, appointment.preferredTimeStart)); const [end, setEnd] = useState(localInput(appointment.endsAt)); const [notes, setNotes] = useState(appointment.adminNotes || ""); const [saving, setSaving] = useState(false);
  async function act(action: string, selectedStart = start, selectedEnd = end) {
    setSaving(true); setNotice("");
    const response = await fetch(`/api/gestion/rendez-vous/${appointment.id}`, {method: "PATCH", headers: {"content-type": "application/json"}, body: JSON.stringify({action, startsAt: selectedStart, endsAt: selectedEnd, adminNotes: notes, customerName, customerEmail, customerPhone, customerAddress, requestType, reason})});
    const data = await response.json(); setSaving(false);
    if (response.ok) {
      setStart(selectedStart); setEnd(selectedEnd); onSaved(data.appointment);
  ...
  const firstStart = availabilityStart(appointment.preferredDate, appointment.preferredTimeStart); const firstEnd = availabilityEnd(appointment.preferredDate, appointment.preferredTimeEnd);
  const secondStart = appointment.alternateDate ? availabilityStart(appointment.alternateDate, appointment.alternateTimeStart) : ""; const secondEnd = appointment.alternateDate ? availabilityEnd(appointment.alternateDate, appointment.alternateTimeEnd) : "";
  return <div className="panel-backdrop" onMouseDown={event => {if (event.target === event.currentTarget) onClose();}}><aside className="detail-panel"><button className="panel-close" onClick={onClose}>×</button><span className={`status-pill status-${appointment.status}`}>{statusLabels[appointment.status]}</span><h2>{customerName}</h2><p className="detail-reason">{requestType} — {reason}</p><div className="contact-buttons"><a href={`tel:${phoneHref(customerPhone)}`}>☎ Appeler</a><a href={`https://wa.me/${phoneHref(customerPhone).replace("+", "")}`} target="_blank">WhatsApp</a><a href={`https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(customerAddress)}`} target="_blank">Itinéraire</a></div><dl><dt>Date souhaitée</dt><dd>{availabilityLabel(appointment.preferredDate, appointment.preferredTimeStart, appointment.preferredTimeEnd)}</dd>{appointment.alternateDate && <><dt>Ancienne 2e proposition</dt><dd>{availabilityLabel(appointment.alternateDate, appointment.alternateTimeStart, appointment.alternateTimeEnd)}</dd></>}<dt>Origine</dt><dd>{appointment.source === "client" ? "Demande client" : "Ajout manuel"}</dd></dl>
    {isNewClientRequest && <section className="availability-actions" aria-labelledby="availability-title"><h3 id="availability-title">Valider la demande</h3><button className="availability-choice" disabled={saving} onClick={() => act("accept", firstStart, firstEnd)}><strong>Valider le rendez-vous</strong><span>{availabilityLabel(appointment.preferredDate, appointment.preferredTimeStart, appointment.preferredTimeEnd)}</span></button>{appointment.alternateDate && <button className="availability-choice alternate" disabled={saving} onClick={() => act("accept", secondStart, secondEnd)}><strong>Valider l’ancienne 2e proposition</strong><span>{availabilityLabel(appointment.alternateDate, appointment.alternateTimeStart, appointment.alternateTimeEnd)}</span></button>}</section>}
    ...
    <label>Date et heure<input type="datetime-local" value={start} onChange={event => setStart(event.target.value)}/></label><label>Fin estimée (facultative)<input type="datetime-local" value={end} onChange={event => setEnd(event.target.value)}/></label><label>Notes internes<textarea rows={4} value={notes} onChange={event => setNotes(event.target.value)}/></label>{notice && <p className="panel-notice">{notice}</p>}<div className="decision-buttons"><button className="accept" disabled={saving} onClick={() => act("save")}>Enregistrer les modifications</button><button className="modify" disabled={saving} onClick={() => act("propose")}>{isNewClientRequest ? "Proposer une autre date" : "Modifier et informer"}</button>{isNewClientRequest && <button className="danger" disabled={saving} onClick={() => act("reject")}>Refuser</button>}{appointment.status !== "cancelled" && <button className="danger ghost" disabled={saving} onClick={() => act("cancel")}>Annuler et informer le client</button>}<button className="delete-button" disabled={saving} onClick={remove}>Supprimer définitivement</button></div></aside></div>;
}
