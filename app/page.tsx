...
const services = [
  ...
  { icon: "cleaning" as const, title: "Nettoyage", text: "Nettoyage de fin de bail, bureaux et fin de chantier. Une remise en état complète, prête pour votre état des lieux.", href: "/services/nettoyage" },
  { icon: "truck" as const, title: "Location de camions", text: "Deux volumes au choix : Iveco Daily 17 m³ ou Citroën Jumper 20 m³ avec hayon, avec ou sans chauffeur.", href: "/services/location-camion" },
  { icon: "delivery" as const, title: "Transport & livraison", text: "Transport de meubles et objets volumineux, livraison à domicile et solutions ponctuelles avec véhicule adapté.", href: "/services/transport-et-livraison" },
  { icon: "assembly" as const, title: "Montage de meubles", text: "Démontage, transport et remontage d’armoires, lits, étagères et autres meubles, avec fixation si nécessaire.", href: "/services/montage-de-meubles" },
  { icon: "recycling" as const, title: "Débarras", text: "Débarras de caves, appartements, bureaux et chantiers avec tri et évacuation responsable des encombrants.", href: "/services/debarras" },
  { icon: "tools" as const, title: "Petits travaux", text: "Bricolage, petites interventions électriques, fixation d’éléments au mur, jardinage et taille de haies.", href: "/services/petits-travaux-jardinage" },
];
...
export default async function Home({searchParams}:{searchParams:SearchParams}) {
  ...
  return <main>
    ...
    <section className="section intro" id="services">
      ...
      <div className="service-grid">{services.map((s, i) => <article className={`service-card ${i < 3 ? "featured" : ""}`} key={s.title}><span className="service-icon"><Icon name={s.icon}/></span><h3>{s.title}</h3><p>{s.text}</p><Link href={s.href}>Découvrir le service <span>→</span></Link></article>)}</div>
    ...
    <section className="cta-section" id="contact"><p className="eyebrow">Besoin d’un coup de main ?</p><h2>Planifiez votre visite ou votre intervention</h2><p>Choisissez vos disponibilités en ligne. Super-Service vous confirme ensuite la date définitive.</p><div className="hero-actions center"><Link className="button button-primary" href="/rendez-vous">Prenez rendez-vous</Link><Link className="button button-outline" href="/devis">Demander un devis</Link><a className="button button-whatsapp" href="https://wa.me/41783223368">WhatsApp</a></div></section>
    <Footer />
    <MobileActions />
  </main>;
}
