...
export const realization = defineType({
  ...
  fields: [
    ...
    defineField({name: "mainImage", title: "Photo", description: "Ajoutez une photo nette. Le cadrage pourra être ajusté directement dans l’image.", type: "seoImage", validation: (rule) => rule.required()}),
    defineField({name: "completedAt", title: "Date de réalisation", type: "date"}),
    defineField({name: "featured", title: "Afficher aussi sur la page d’accueil", description: "Activez cette option uniquement pour vos meilleures photos.", type: "boolean", initialValue: false}),
