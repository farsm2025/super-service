...
export function Testimonials({items,reviewSent=false}:{items:DisplayTestimonial[];reviewSent?:boolean}){
  ...
  return <section className="testimonials" id="avis">
    {reviewSent?<div className="review-notice success" role="status"><strong>Merci, votre avis a bien été envoyé.</strong><span>Il a été transmis à Super-Service et sera publié après approbation.</span></div>:null}
    <div className="testimonial-head"><div><p className="eyebrow light">Avis clients <span className="reviews-count light">{items.length}</span></p><h2>Leur confiance est notre meilleure référence</h2></div><div className="rating"><strong>{average.toLocaleString("fr-CH",{minimumFractionDigits:1,maximumFractionDigits:1})}</strong><span>★★★★★</span><small>Avis validés par Super-Service</small></div></div>
